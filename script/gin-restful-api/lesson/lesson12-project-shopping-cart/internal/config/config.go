...
func NewConfig() *Config {
	...
	mailProviderType := utils.GetEnv("MAIL_PROVIDER", "mailtrap")
	if mailProviderType == "mailtrap" {
		mailtrapConfig := map[string]any{
			"mail_sender":      utils.GetEnv("MAILTRAP_MAIL_SENDER", "admin@golangwithginframeword.com"),
			"name_sender":      utils.GetEnv("MAILTRAP_NAME_SENDER", "Golang Tutorial"),
			"mailtrap_url":     utils.GetEnv("MAILTRAP_URL", "https://sandbox.api.mailtrap.io/api/send/4421683"),
			"mailtrap_api_key": utils.GetEnv("MAILTRAP_API_KEY", "ab59a1d4b2f63367156f991a7f9ab511"),
		}
